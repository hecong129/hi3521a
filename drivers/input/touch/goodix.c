...
unsigned char gpio_i2ctp_read(unsigned char devaddress,
...
static void gpio_i2ctp_write(unsigned char devaddress,
                             unsigned char addressh,
                             unsigned char addressl, unsigned char data)
{
    int i;
    spin_lock(&gpioi2c_lock);
    devaddress &= 0xFE;
...
void gpio_i2ctp_read_buf(unsigned char devaddress, unsigned char addressh,
                         unsigned char addressl, unsigned char *buf,
                         int size)
{
    int i = 0;
    spin_lock(&gpioi2c_lock);
...
void gpio_i2ctp_write_buf(unsigned char devaddress, unsigned char addressh,
                          unsigned char addressl, unsigned char *buf,
                          int size)
{
    int i = 0;
...
static const u8 gt911_table[186] = {
    ...
    
};

// 将GPIO信息从该结构体传入 
static struct tp_platform_data {
    u32 scl;                    // SCL PIN  地址
    u32 sda;                    // SDA PIN  地址
    u32 rst;                    // 复位 PIN 地址
    u32 irq;                    // 中断 PIN 地址 
    ...
    int irq_num;                // 中断号
    int irq_type;               // 中断类型
    int use_poll;               // 是否使用轮询定时器, 当前临时硬件有问题, 一套代码同时兼容中断和轮询.
    int use_rep;                // 是否重复上报按键, 不释放时,按间隔上报value=2的键值, 第一次value为1,释放value为0

    //int use_multy; 多点触控
};

struct tp_platform_data tp_data = {
    .scl = 0x12220000,
    .sda = 0x12220000,
    .rst = 0x12220000,
    .irq = 0x12220000,
...
static void gt911_device_release(struct device *dev)
{
    return;
}
...
static int goodix_i2c_read(u8 addr, u16 reg, u8 * buf, int len)
{
...
static int goodix_scan_handler(void *dev_id)
{
    static u8 end_cmd[] = {
        GOODIX_READ_COOR_ADDR >> 8,
        GOODIX_READ_COOR_ADDR & 0xff,
    ...
    struct goodix_ts_data *ts = dev_id;
    if (gpio_i2ctp_read(DEVICE_ADDR, end_cmd[0], end_cmd[1]) & 0x80) {
        goodix_process_events(ts);
        gpio_i2ctp_write(DEVICE_ADDR, end_cmd[0], end_cmd[1], end_cmd[2]);
    }
    return 0;
...
static void goodix_read_config(struct goodix_ts_data *ts)
{
    ...
    error = goodix_i2c_read(DEVICE_ADDR, GOODIX_REG_CONFIG_DATA,
                            config, GOODIX_CONFIG_MAX_LENGTH);
    if (error) {
...
static int goodix_read_version(u8 addr, u16 * version)
{
    ...
    error =
        goodix_i2c_read(DEVICE_ADDR, GOODIX_REG_VERSION, buf, sizeof(buf));
    if (error) {
...
static int gt911_open(struct input_dev *dev)
{
    ...
    if (ts->use_poll) {
        goodix_scan_handler(ts);
        schedule_delayed_work(&ts->dwork, msecs_to_jiffies(100));
        INF("open here\n");
    } else {
...
static int gt911_close(struct input_dev *dev)
{
    struct goodix_ts_data *ts = input_get_drvdata(dev);
    if (ts->use_poll) {
        INF("close here\n");
...
static int goodix_request_input_dev(struct goodix_ts_data *ts)
{
  ...
  ts->input_dev->evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS) | BIT_MASK(EV_REP);
  __set_bit(EV_REP,ts->input_dev->evbit);
    ...
    ts->input_dev->open = gt911_open;
    ts->input_dev->close = gt911_close;
...
{
    struct goodix_ts_data *ts =
        container_of(work, struct goodix_ts_data, dwork.work);
    //printk("ts_work_func\n");
    goodix_scan_handler(ts);
    schedule_delayed_work(&ts->dwork, msecs_to_jiffies(10));
}
...
static int goodix_ts_probe(struct platform_device *pdev)
{
    ...
    u16 version_info;
    u32 reg = 0;
    struct tp_platform_data *pdata = pdev->dev.platform_data;
    ...
    ts->use_poll = pdata->use_poll;
    if (pdata->use_poll == 0) {
        if (devm_request_threaded_irq
            (&gt911_device.dev, 90, NULL, &goodix_ts_irq_handler,
             goodix_irq_flags[0] | IRQF_ONESHOT, "gt911", ts)) {
        ...
        DBG("scan init ok!\n");
        INIT_DELAYED_WORK(&ts->dwork, ts_work_func);
    }
